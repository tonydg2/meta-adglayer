...
const char *tx_channel_names[] = { "dma_proxy_tx_0", "dma_proxy_tx_1" };
const char *rx_channel_names[] = { "dma_proxy_rx_0", "dma_proxy_rx_1" };
...
//adg	pthread_attr_t tattr_tx;
	 ...
	 */
//adg	pthread_attr_init (&tattr_tx);
//adg	pthread_attr_getschedparam (&tattr_tx, &param);

	/* Set the transmit priority to the lowest
	 */
//adg	param.sched_priority = newprio;
//adg	pthread_attr_setschedparam (&tattr_tx, &param);
...
*/
  ...
  printf("ADG DEBUG0 rx chan setup\n");
  ...
  printf("ADG DEBUG1 setup threads\n");
	setup_threads(&num_transfers);

  sleep(1);
  printf("ADG DEBUG2 pthread join\n");
